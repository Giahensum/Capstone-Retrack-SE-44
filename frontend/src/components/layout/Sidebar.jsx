...
import { LayoutDashboard, Package, Truck, BarChart3, Factory, Users, Settings, LogOut, Menu, X, Recycle } from 'lucide-react';
...
export function Sidebar() {
    ...
    return (<aside className={cn('flex flex-col h-screen bg-slate-900 border-r border-slate-800 transition-all duration-300', collapsed ? 'w-16' : 'w-60')}>
      {/* Logo */}
      <div className="flex items-center gap-3 px-4 py-5 border-b border-slate-800">
        <div className="w-8 h-8 bg-emerald-500 rounded-lg flex items-center justify-center flex-shrink-0">
          <Recycle size={18} className="text-white"/>
        </div>
        {!collapsed && (<span className="font-bold text-slate-100 text-lg tracking-tight">ReTrack</span>)}
        <button onClick={() => setCollapsed(!collapsed)} className="ml-auto text-slate-400 hover:text-slate-100 transition-colors">
      ...
      <nav className="flex-1 py-4 px-2 space-y-1">
        {navItems.map((item) => {
            ...
            return (<Link key={item.href} to={item.href} className={cn('flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-150', active
                    ? 'bg-emerald-500/15 text-emerald-400 border border-emerald-500/20'
                    : 'text-slate-400 hover:bg-slate-800 hover:text-slate-100')} title={collapsed ? item.label : undefined}>
      ...
      <div className="border-t border-slate-800 p-3">
        {!collapsed && (<div className="px-2 mb-2">
            <p className="text-xs font-semibold text-slate-100 truncate">{user?.fullName}</p>
            <p className="text-xs text-slate-400 truncate">{user?.role}</p>
          </div>)}
        <button onClick={handleLogout} className={cn('flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-red-400 hover:bg-red-500/10 transition-colors w-full', collapsed && 'justify-center')} title={collapsed ? 'Đăng xuất' : undefined}>
          <LogOut size={16}/>
          {!collapsed && 'Đăng xuất'}
        </button>
      </div>
