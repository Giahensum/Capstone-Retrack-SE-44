...
export async function login(email, password) {
  return request("/api/auth/login", { method: "POST", body: { email, password }, token: null });
}
...
function mapOrder(order, partners) {
  ...
  const status = order.status === "ACCEPTED" && transportStatus === "DELIVERED"
    ? "DELIVERED"
    : order.status === "ACCEPTED" && ["PICKED_UP", "ON_THE_WAY"].includes(transportStatus)
      ? "IN_TRANSIT"
      : order.status;
  const statusSteps = status === "REJECTED"
    ? ["ACCEPTED", "IN_TRANSIT", "DELIVERED", "RECEIVED", "WEIGHED", "REJECTED"]
    : ["ACCEPTED", "IN_TRANSIT", "DELIVERED", "RECEIVED", "WEIGHED", "VERIFIED", "PAID"];
  const reached = statusSteps.indexOf(status);
  ...
  return {
    ...
    rating: partner?.rating ? { stars: partner.rating, comment: partner.comment || "" } : null,
...
export async function performFactoryAction(type, payload) {
  ...
  case "TOGGLE_DEMAND":
    ...
    case "REJECT_OFFER":
      return request(`/api/factory/marketplace/offers/${id}/reject`, { method: "POST", body: { accept: false, reason: payload.reason } });
    case "RECEIVE":
