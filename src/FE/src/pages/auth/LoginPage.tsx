...
export default function LoginPage() {
  ...
  // ── Helpers ──────────────────────────────────────────
  const handleSuccess = (data: AuthResponseDto) => {
    login(data.token, {
      userId: data.userId,
      email: data.email,
  ...
  const loginMutation = useMutation({
    mutationFn: async () => {
      const res = await api.post<{ success: boolean; data: AuthResponseDto }>(
        '/auth/login', form
      )
  ...
  const googleLogin = useGoogleLogin({
    ...
    onSuccess: async (tokenResponse) => {
      try {
        ...
        const googleMutation = await api.post<{ success: boolean; data: AuthResponseDto }>(
          '/auth/google',
    ...
    <div className="min-h-screen bg-[#f8f9ff] flex">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 lg:p-12 overflow-y-auto">
        ...
        <div className="w-full max-w-md">
          ...
          <button
            onClick={() => googleLogin()}
            className="w-full flex items-center justify-center gap-3 border-2 border-[#e5e7eb] rounded-2xl py-3.5 px-6 font-semibold text-[#1f2937] hover:bg-[#f3f4f6] hover:border-[#d1d5db] transition-all duration-200 mb-6 bg-white shadow-sm"
          >
          ...
          <form onSubmit={(e) => { e.preventDefault(); loginMutation.mutate() }} className="space-y-4">
            <Input
              ...
              required
              className="!bg-white !border-[#e5e7eb] !text-[#1f2937] focus:!border-[#446900] focus:!ring-[#446900]/20 rounded-xl"
            />
            ...
            <div className="relative">
              ...
              <button
                ...
                className="absolute right-3 bottom-2.5 text-[#9ca3af] hover:text-[#446900] transition-colors"
              >
            ...
            <button
              ...
              className="w-full bg-[#a3e635] hover:bg-[#bef264] text-[#0b1c30] font-bold py-3.5 rounded-2xl transition-all duration-200 flex items-center justify-center gap-2 shadow-lg shadow-[#a3e635]/30 hover:shadow-[#a3e635]/50 disabled:opacity-60 disabled:cursor-not-allowed text-base"
            >
          ...
          <details className="mt-8 border border-[#e5e7eb] rounded-2xl overflow-hidden">
            ...
            <div className="p-4 space-y-2">
              {SEED_ACCOUNTS.map((a) => (
                <button
                  key={a.role}
                  onClick={() => fillAccount(a.email, a.pass)}
                  className="w-full text-left flex items-center justify-between px-3 py-2 rounded-xl hover:bg-[#f3f4f6] transition-colors group"
                >
                  <div>
